...
Public Class FX3ConfigGUI
    ...
    Private Sub SetupToolTips()
        ...
        tip.SetToolTip(frequencyInput, "Set the SPI clock frequency when using hardware SPI. Valid range ~1KHz - 33MHz")
        tip.SetToolTip(polarityInput, "Set the SPI clock polarity (CPOL)")
        tip.SetToolTip(phaseInput, "Set the SPI data sample phase (CPHA)")
        tip.SetToolTip(chipSelectPolarityInput, "Set the chip select polarity (idle high or idle low). Most devices are idle high")
        tip.SetToolTip(chipSelectControlInput, "Set the chip select control mode. This should usually be left in hardware control mode")
        tip.SetToolTip(leadTimeInput, "Set the delay (in SPI clocks) from the CS idle-active edge to the first SPI clock")
        tip.SetToolTip(lagTimeInput, "Set the delay (in SPI clocks) from the last SPI clock to the CS active-idle edge")
        ...
        tip.SetToolTip(sensorInput, "Set the selected sensor type")
        tip.SetToolTip(DutInput, "Set the selected DUT type")
        tip.SetToolTip(regEndiannessInput, "Set the endianness for 32-bit registers which are split across multiple addresses")
        tip.SetToolTip(WatchdogEnable, "Enable the watchdog timer on the EVAL-ADIS-FX3 micro")
        tip.SetToolTip(WatchdogTimeout, "Set watchdog timeout period on the EVAL-ADIS-FX3 micro")
    ...
    Private Sub SetConfig_Click(sender As Object, e As EventArgs) Handles SetConfig.Click
        ...
        m_TopGUI.SelectedPersonalityLabel = DutPersonality.CUSTOM_PERSONALITY_STRING
